...
from Environment.diff_Robot_gym_v2_p2p import DiffRobotEnv 
import sys
...
env = DiffRobotEnv(mode ="simple")
logDir="traing_logs"
runName="sac-v2-raw-lidar-static-p2p-{}".format(datetime.now())
callback=EpisodeRewardLoggingCallback(env,logDir,verbose=1,evalfrequency=10,run_name=runName)



model = SAC("MlpPolicy", env, verbose=1,tensorboard_log="{}/{}".format(logDir,runName))

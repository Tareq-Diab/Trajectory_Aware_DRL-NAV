...
the environemnt state is image and sensor vector (image:1x2x50x50 scaler:1x5)
"""
import rospy
...
packagepath=RosPack()
try : 
    path=packagepath.get_path("simulation_env")+"/src/Environment"
    print(path) 
except : 
    path="Environment"
...
class Environment ():
    ...
    def __init__(self,elemntsFile:str="/dynamicElements_p2p.Yaml",controlFrequency=20,number_actors=5, **kwargs):
        ...
        self.nS,self.nA=Dict({"image": Box(low=0,high=1,shape=(2,50,50)), "scalers": Box(low=-1,high=1,shape=(5,))}),2
        self.observation_space=Dict({"image": Box(low=0,high=1,shape=(2,50,50)), "scalers": Box(low=-1,high=1,shape=(5,))})
    ...
    def __getObservation(self): 
        ...
        self.opservation_return["image"]= result  
        self.opservation_return["scalers"]=self.opservation
        return self.opservation_return
    
    def __getThetaRobotGoal(self): 

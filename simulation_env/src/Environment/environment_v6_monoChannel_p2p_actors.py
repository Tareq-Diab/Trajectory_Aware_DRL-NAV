...
the environemnt state is image and sensor vector (image:1x50x50 scaler:1x5)
"""
import rospy
...
from Environment.dynamicEnvironmentManger import HumanoidActorsManger 
...
class Environment ():
    """_summary_
    """
    def __init__(self,elemntsFile:str="/dynamicElements_p2p.Yaml",controlFrequency=20,number_actors=5, **kwargs):
        ...
        costmapSubscriber=rospy.Subscriber("/costmap_node/costmap/costmap_updates",OccupancyGridUpdate,callback=self.costmapCB) 
        ...
        self.goal={"x":-9,"y":0} 
        self.old_distance_to_goal=0
        self.reachGoalThreshold=0.5
        #-------------------------------------------variables----------------------------------------
        self.distance_max=abs(self.goal["x"]) 
        self.nS,self.nA=Dict({"image": Box(low=0,high=1,shape=(1,50,50)), "scalers": Box(low=-1,high=1,shape=(5,))}),2
        self.observation_space=Dict({"image": Box(low=0,high=1,shape=(1,50,50)), "scalers": Box(low=-1,high=1,shape=(5,))})
        ...
        self.opservation_return={"image": Box(low=0,high=1,shape=(1,50,50)), "scalers": Box(low=-1,high=1,shape=(5,))}
        ...
        #---------------------------------------------- p2p ----------------------------------------------------
        self.goalrandomizer=GoalRandomizer(-9,9,1,obstacle_yaml=path+"/obstacles.yaml")
        self.ActorManger= HumanoidActorsManger(actor_number=number_actors,forbiddenArea=[0,0,3],pathsyaml=path+"/nav_goals.yaml")
    ...
    def reset(self):
        ...
        self.done=False
        self.colided=False
        self.goal=self.goalrandomizer.getRandomGoal() 
        ...
        print("reseted goal {} distance {}".format(self.goal,self.distance_max))
    ...
    def __getObservation(self): 
        ...
        #TODO - calculate theta to the goal
        ...
        distance_reward = 1 - (self.distance_to_goal / self.distance_max)**0.5 #FIXME - whyu substract 1  ?  this i s a mistake #TODO -  EDIT THIS
        ...
        self.opservation_return["image"]= self.rotate_image(self.image,yaw_robot*(180/np.pi))
        self.opservation_return["scalers"]=self.opservation
    ...
    def __randomizeEnvironment(self,dynamicElements:dict=None): #TODO  ENCAPSULATE 
        """_summary_
    ...
    def scanCB(self,data):
        ...
        if np.min(data.ranges) < self.minLaserRangeBeforCollision:
            self.collision=True 
        self.laser_data=np.array([np.min(data.ranges[i*30:i*30+30])  for i in range(12) ] )

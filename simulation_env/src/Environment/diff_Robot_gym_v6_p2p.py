...
from Environment.environment_v6_p2p import Environment as env_p2p
from Environment.environment_v6_p2p_actors import Environment as env_p2p_actors
...
class DiffRobotEnv(gym.Env):
    ...
    def __init__(self,mode="actors",nactors=5):
        super().__init__()
        ...
        self.observation_space =  Dict({"image": Box(low=0,high=255,shape=(2,50,50),dtype=np.uint8), "scalers": Box(low=-1,high=1,shape=(5,))})


    def step(self, action):
        observation, reward, terminated,info=self.env.step(action)

        self.observation["image"]=np.reshape((observation["image"]*2.55),(2,50,50)).astype(np.uint8)
